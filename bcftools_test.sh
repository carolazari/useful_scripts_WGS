...
# === STEP 6: MERGE DOWNSAMPLED PER-CHROMOSOME BCFs IN BATCHES ===
echo "[INFO] Merging downsampled per-chromosome BCFs in batches..."

FILES=($(ls "$CHROM_DIR"/*.bcf))
TOTAL_FILES=${#FILES[@]}
TMP_MERGED=()

if [ $TOTAL_FILES -eq 0 ]; then
    echo "[ERROR] No per-chromosome BCF files found to merge."
    exit 1
fi
...
for (( i=0; i<$TOTAL_FILES; i+=BATCH_SIZE )); do
    ...
    BATCH_OUTPUT="$OUTPUT_DIR/batch_merge_$i.bcf"
    echo "[INFO] Merging batch starting at file $i (count: ${#BATCH_FILES[@]})..."
    bcftools merge -O b -o "$BATCH_OUTPUT" "${BATCH_FILES[@]}"
    bcftools index "$BATCH_OUTPUT"
    TMP_MERGED+=("$BATCH_OUTPUT")
done

if [ ${#TMP_MERGED[@]} -eq 0 ]; then
    echo "[ERROR] No batch merged BCF files created. Exiting."
    exit 1
elif [ ${#TMP_MERGED[@]} -eq 1 ]; then
    echo "[INFO] Only one batch merge file, copying to final output..."
    cp "${TMP_MERGED[0]}" "$MERGED_BCF"
    bcftools index "$MERGED_BCF"
else
    echo "[INFO] Merging all batch merges into final BCF..."
    bcftools merge -O b -o "$MERGED_BCF" "${TMP_MERGED[@]}"
    bcftools index "$MERGED_BCF"
fi
